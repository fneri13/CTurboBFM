...
//     FloatType forceMagNew = Kmach * _relativeVelocityCylindric.dot(_relativeVelocityCylindric) * M_PI * _deviationAngle / _pitch / std::abs(_normalCamberTangential) / _blockage;
...
//     FloatType forceMagOld = inviscidForce(i, j, k).magnitude();

//     FloatType forceInviscidMagnitude = forceMagOld - (deltaTotEnthalpy_dm_actual - deltaTotEnthalpy_dm_ref) / (deltaTotEnthalpy_dm_actual + deltaTotEnthalpy_dm_ref) * forceMagNew * _streamwiseCoeff;
...
FloatType CSourceBFMNeri::computeCompressibilityCorrection(const Vector3D& relativeVelocityCylindric, const StateVector& primitive) {
