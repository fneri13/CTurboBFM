...
BoundaryInlet2D::BoundaryInlet2D(const Config &config, const Mesh &mesh, FluidBase &fluid, BoundaryIndices boundIndex, std::string inletFile)
...
StateVector BoundaryInlet2D::computeBoundaryFlux(StateVector internalConservative, Vector3D surface, Vector3D midPoint, std::array<size_t, 3> indices, const FlowSolution &flowSolution, const size_t iterCounter) {
