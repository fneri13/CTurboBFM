...
BoundaryInletSupersonic::BoundaryInletSupersonic(const Config &config, const Mesh &mesh, FluidBase &fluid, BoundaryIndices boundIndex, std::vector<FloatType> inletValues)
    : BoundaryBase(config, mesh, fluid, boundIndex) {
        _boundaryValues = inletValues;
    }


StateVector BoundaryInletSupersonic::computeBoundaryFlux(StateVector internalConservative, Vector3D surface, Vector3D midPoint, std::array<size_t, 3> indices, const FlowSolution &flowSolution, const size_t iterCounter) {
