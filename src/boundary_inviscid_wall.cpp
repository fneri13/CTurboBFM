...
BoundaryInviscidWall::BoundaryInviscidWall(const Config &config, const Mesh &mesh, FluidBase &fluid, BoundaryIndices boundIndex)
    : BoundaryBase(config, mesh, fluid, boundIndex) {}


    StateVector BoundaryInviscidWall::computeBoundaryFlux(StateVector internalConservative, Vector3D surface, Vector3D midPoint, std::array<size_t, 3> indices, const FlowSolution &flowSolution, const size_t iterCounter) {
