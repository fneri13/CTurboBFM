...
TEST(CMeshTest, TestComputeVolumes3D) {
    ...
    std::vector<FloatType> expectedVolumes = {0.125,
                                              0.25,
                                              0.5,
                                              1.0};                                      
...
TEST(CMeshTest, TestReadMesh3D) {
    ...
    std::vector<size_t> iTest {0,1,2};
    std::vector<size_t> jTest {0,1,2};
    std::vector<size_t> kTest {0,1,2};

    for (size_t i=0; i<iTest.size(); i++){
        for (size_t j=0; j<jTest.size(); j++){
            for (size_t k=0; k<kTest.size(); k++){
                ASSERT_DOUBLE_EQ(vertices(iTest[i],jTest[j],kTest[k]).x(), static_cast<double>(iTest[i]));
                ASSERT_DOUBLE_EQ(vertices(iTest[i],jTest[j],kTest[k]).y(), static_cast<double>(jTest[j]));
                ASSERT_DOUBLE_EQ(vertices(iTest[i],jTest[j],kTest[k]).z(), static_cast<double>(kTest[k]));
            }
        }
    }
}
...
TEST(CMeshTest, TestDualGrid3DCartesian) {
    ...
    std::vector<Vector3D> expectedNodes {{0.0, 0.0, 0.0},
                                         {0.5, 0.0, 0.0},
                                         {0.5, 0.5, 0.0},
                                         {0.5, 0.5, 0.5},
                                         {7.5, 5.5, 3.5}};
